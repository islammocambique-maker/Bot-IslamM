...
// Importa configuração (funciona em Node e Browser)
let CONFIG;
if (typeof require !== 'undefined') {
    const { config } = require('./config-loader.js');
    CONFIG = config;
} else {
    CONFIG = window.BotConfig || window.CONFIG;
}
...
// ============================================
// INTEGRAÇÃO GEMINI (IA FALLBACK) - SEGURA
// ============================================
...
async function perguntarGemini(pergunta) {
    if (!CONFIG.GEMINI.ATIVO || !CONFIG.GEMINI.API_KEY) {
        console.log("Gemini desativado ou sem API Key configurada");
        return null;
    }
